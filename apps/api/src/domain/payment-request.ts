...
export function assertSubmittable(request: PaymentRequest): void {
  ...
  if (
    !/^\d+(\.\d{1,4})?$/.test(String(request.amount)) ||
    Number(request.amount) <= 0
  ) {
    throw new Error(
      "Amount must be a positive decimal with at most four decimal places",
    );
  }
  if (!/^[A-Z]{3}$/.test(String(request.currency)))
    throw new Error("Currency must be a three-letter ISO code");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(normalizeDate(request.dueDate)))
...
export function formatTicketNumber(
  ...
  if (sequence < 1n || sequence > 999999n)
    throw new Error("Ticket sequence is outside its supported range");
