...
export class FinanceControlService {
  ...
  async list(actor: Principal, filter?: {departmentId?:string;category?:string;page?:number;pageSize?:number}) {
    ...
    const q = await this.db.pool.query(
      ...
      WHERE pr.created_by<>$1 AND pr.status IN('APPROVED','FINANCE_CHECK','FINANCE_HOLD','READY_FOR_PAYMENT') AND($2::uuid IS NULL OR pr.department_id=$2)AND($3::text IS NULL OR pr.category=$3)
      ), page_rows AS (
  ...
  private async duplicateStatus(c: any, id: string, r: any) {
    const hash = await c.query(
      `SELECT 1 FROM payment_documents d JOIN payment_documents other ON other.sha256=d.sha256 AND other.payment_request_id<>d.payment_request_id AND other.removed_at IS NULL AND other.security_status='CLEAN' AND other.storage_binding_state='VERSION_BOUND'
    JOIN payment_requests pr ON pr.id=other.payment_request_id AND pr.status NOT IN('DRAFT','REJECTED','CANCELLED') WHERE d.payment_request_id=$1 AND d.removed_at IS NULL AND d.security_status='CLEAN' AND d.storage_binding_state='VERSION_BOUND' LIMIT 1`,
      [id],
    );
  ...
  private async checks(c: any, request: any, run: any): Promise<CheckResult[]> {
    ...
    const required = Array.isArray(f.required_evidence)
      ...
      types = new Set(
        (
          await c.query(
            "SELECT document_type FROM payment_documents WHERE payment_request_id=$1 AND removed_at IS NULL AND security_status='CLEAN' AND storage_binding_state='VERSION_BOUND'",
            [request.id],
          )
