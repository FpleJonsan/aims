...
async function main(){
  const config=loadWorkerConfig(),workloads:PollWorkload[]=[];let outboxDb:Postgres|undefined,documents:DocumentScanWorker|undefined;
  if(config.telegramEnabled){outboxDb=new Postgres();const outbox=new ApprovalOutboxService(outboxDb,new TelegramApprovalChannel(process.env.TELEGRAM_BOT_TOKEN!));workloads.push({name:"telegram_outbox",poll:()=>outbox.dispatch(config.batchSize)})}
  ...
  const shutdown=()=>{shutdownRequested=true;loop.stop();documents?.stop();shutdownTimer??=setTimeout(()=>{process.stderr.write(`${JSON.stringify({timestamp:new Date().toISOString(),component:"aims-worker",event:"worker_shutdown_deadline_exceeded"})}\n`);process.exit(1)},config.shutdownGraceMs)};
