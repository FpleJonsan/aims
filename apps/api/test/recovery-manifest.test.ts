...
const valid=()=>({specificationVersion:"1",environment:"isolated-restore",databaseRecoveryReference:"db-evidence-1",databaseRecoveryPoint:"2026-09-01T00:00:00Z",objectRecoveryReference:"object-evidence-1",applicationRelease:"release-63",schemaVersion:63,latestMigrationId:"063_p20_5b_finance_master_users",expectedRecoveryGeneration:"123e4567-e89b-42d3-a456-426614174000",generationAdvancementEvidenceReference:"generation-evidence-1",createdAt:"2026-09-01T01:00:00Z",integrity:{algorithm:"PROVIDER_ATTESTATION_REQUIRED",evidenceReference:"provider-attestation-required"},operatorMetadata:{incidentId:"INC-1"}});

test("valid recovery manifest is parsed and bounded",()=>assert.equal(parseRecoveryManifest(valid()).schemaVersion,63));
