...
test("published AI configuration OFF is authoritative even with a valid credential configured", async () => {
  const previous = { ...process.env };
  process.env.STORAGE_DRIVER = "local";
  process.env.MALWARE_SCANNER_DRIVER = "deterministic-local";
...
test("a missing schema version table is not ready", async () => {
  const previous = { ...process.env }; process.env.STORAGE_DRIVER = "local"; process.env.MALWARE_SCANNER_DRIVER = "deterministic-local"; process.env.TELEGRAM_APPROVAL_ENABLED = "false";
  const pool = { query: async (query: string) => { if (query.includes("aims_schema_version")) throw new Error("missing"); if (query.includes("configuration_versions")) return { rows: [{ payload: { enabled: false } }] }; return { rows: [{}] }; } };
