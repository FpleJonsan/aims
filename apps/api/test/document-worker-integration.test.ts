...
test("compiled worker process starts independently, idles, and shuts down gracefully",async()=>{
 ...
 try{
  await new Promise<void>((resolve,reject)=>{let settled=false;const deadline=setTimeout(()=>{settled=true;reject(new Error(`worker did not start: ${stderr}`))},5000);const inspect=()=>{if(settled)return;if(stdout.includes('"event":"worker_started"')){settled=true;clearTimeout(deadline);resolve()}else if(child.exitCode!==null){settled=true;clearTimeout(deadline);reject(new Error(`worker exited before startup: ${stderr}`))}else setTimeout(inspect,20)};inspect()});
