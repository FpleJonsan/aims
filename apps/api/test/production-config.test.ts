...
import {
  ...
  redactSensitiveText,
  SERVER_SECRET_NAMES,
} from "../src/infrastructure/configuration/secret-boundary.js";
...
test("production fails closed without an approved corporate authentication adapter", () => {
  ...
  assert.throws(
    () => validateProductionConfig({ ...configured, NODE_ENV: "production" }),
...
test("production rejects missing object storage, missing scanner, and deterministic local scanner", () => {
  ...
  assert.throws(
    () => validateProductionConfig({ ...base, STORAGE_DRIVER: "" }),
    /private object-storage adapter/,
  );
  assert.throws(
    () =>
...
test("production rejects placeholder and incomplete database credentials", () => {
  ...
  assert.throws(
    () =>
      validateProductionConfig({
        ...production,
        DATABASE_URL:
          "postgresql://aims_app:replace_with_password@db.internal/aims_production?sslmode=verify-full",
      }),
  ...
  assert.throws(
    () =>
      validateProductionConfig({
        ...local,
        DATABASE_URL: "postgresql://db.internal/aims",
      }),
    /injected runtime credential/,
  );
});
...
test("production database identity, TLS, and role separation fail closed", () => {
  ...
  assert.throws(
    () =>
      validateProductionConfig({
        ...base,
        AIMS_EXPECTED_DATABASE: "aims_competition",
      }),
    /AIMS_EXPECTED_DATABASE|isolated non-local database/,
  );
  assert.throws(
    () =>
      validateProductionConfig({
        ...base,
        DATABASE_URL: productionUrl("aims_app").replace(
          "?sslmode=verify-full",
          "",
        ),
      }),
  ...
  assert.throws(
    () =>
      validateProductionConfig({
        ...base,
        DATABASE_URL: productionUrl("postgres"),
      }),
  ...
  assert.throws(
    () =>
      validateProductionConfig({
        ...base,
        PAYMENT_DATABASE_URL: productionUrl("aims_finance_runtime"),
      }),
  ...
  assert.throws(
    () =>
      validateProductionConfig({
        ...base,
        DATABASE_URL: productionUrl("aims_app", "wrong_database"),
      }),
    /AIMS_EXPECTED_DATABASE/,
  );
});

test("secret redaction protects structured and unstructured boundaries", () => {
  const canary = "p5-canary-not-a-real-secret",
    connection = `postgresql://aims:${canary}@localhost:5432/aims`;
  const serialized = JSON.stringify(
    redactSensitiveData({
      authorization: `Bearer ${canary}`,
      nested: { databaseUrl: connection, note: `password=${canary}` },
    }),
  );
  assert.doesNotMatch(serialized, new RegExp(canary));
  assert.match(serialized, /REDACTED/);
  assert.doesNotMatch(
    redactSensitiveText(`failed ${connection}`),
    new RegExp(canary),
  );
});
...
test("server secret catalogue excludes browser-public configuration", () => {
  ...
  assert.equal(
    SERVER_SECRET_NAMES.some((name) => name.startsWith("NEXT_PUBLIC_")),
    false,
  );
});
...
test("AI OFF has no OpenAI credential dependency", () => {
  assert.doesNotThrow(() =>
    validateProductionConfig({ ...local, AI_MASTER: "OFF" }),
  );
});

test("AI OFF ignores stale secrets and every malformed provider-only value", () => {
  assert.doesNotThrow(() =>
    validateProductionConfig({
      ...local,
      AI_MASTER: "OFF",
      ...
      AI_RETRY_BASE_DELAY_MS: "invalid",
...
test("AI ON requires provider configuration", () => {
  assert.throws(
    () => validateProductionConfig({ ...local, AI_MASTER: "ON" }),
    /OPENAI_API_KEY/,
  );
});
test("AI ON rejects invalid reliability controls while AI OFF does not require them", () => {
  const enabled = {
    ...local,
    AI_MASTER: "ON",
  ...
  for (const [name, value] of [
    ...
    ["AI_MAX_RETRIES", "-1"],
    ["AI_RETRY_BASE_DELAY_MS", "NaN"],
    ["AI_MAX_RESPONSE_BYTES", "999999999"],
  ])
    assert.throws(
      () => validateProductionConfig({ ...enabled, [name]: value }),
      new RegExp(name),
    );
  assert.doesNotThrow(() =>
    validateProductionConfig({
...
test("enabled Telegram requires all channel secrets", () => {
  assert.throws(
    () =>
      validateProductionConfig({ ...local, TELEGRAM_APPROVAL_ENABLED: "true" }),
    /TELEGRAM_BOT_TOKEN/,
  );
});
...
test("production Telegram secrets and webhook must be strong and HTTPS", () => {
  ...
  assert.doesNotThrow(() =>
    validateProductionConfig({
      ...base,
      TELEGRAM_WEBHOOK_URL: "http://example.test/hook",
    }),
  );
  assert.equal(
    validateProductionConfig({
...
test("staging and competition are explicitly separated", () => {
  ...
  assert.equal(
    validateProductionConfig({ ...local, AIMS_ENVIRONMENT: "competition" })
      .identity,
    "COMPETITION_HEADER",
  );
});
