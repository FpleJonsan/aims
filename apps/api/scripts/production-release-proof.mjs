...
try{
 ...
 await client.end();
 if(Number(health.rows[0]?.version)!==71||health.rows[0]?.migration_id!=="071_p20_7a_enterprise_ui_contracts")throw new Error("runtime schema check failed");
 ...
 console.log(JSON.stringify({result:"PASS",environment:"isolated-production-proof",schema:71,migration:"071_p20_7a_enterprise_ui_contracts",migrationFiles:71,executedFiles:57,deferredFixtureFiles:14,filteredMixedFiles:2,noDemoRecords:true,systemBootstrap:true,privileges:true,runtimeSchemaCheck:true,developmentFixture:true,developmentFixtureIdempotent:true}));
