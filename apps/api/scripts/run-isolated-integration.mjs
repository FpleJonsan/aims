...
try{
  ...
  const migrations=(await readdir(path.join(apiRoot,"migrations"))).filter(name=>/^\d{3}_.*\.sql$/.test(name)&&Number(name.slice(0,3))<=67).sort();
  if(migrations.length!==67||!migrations[0].startsWith("001_")||!migrations.at(-1).startsWith("067_"))throw new Error("expected immutable migration chain 001-067");
  ...
  console.log(JSON.stringify({result:"PASS",database:"isolated-disposable",schema:67,tests:requested.length,cleanup:"pending"}));
}finally{
