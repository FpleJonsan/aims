...
try{
  ...
  const migrations=(await readdir(path.join(apiRoot,"migrations"))).filter(name=>/^\d{3}_.*\.sql$/.test(name)&&Number(name.slice(0,3))<=71).sort();
  if(migrations.length!==71||!migrations[0].startsWith("001_")||!migrations.at(-1).startsWith("071_"))throw new Error("expected immutable migration chain 001-071");
  ...
  console.log(JSON.stringify({result:"PASS",database:"isolated-disposable",schema:71,tests:requested.length,cleanup:"pending"}));
}finally{
