...
try{
  ...
  const migrations=(await readdir(path.join(apiRoot,"migrations"))).filter(name=>/^\d{3}_.*\.sql$/.test(name)&&Number(name.slice(0,3))<=63).sort();
  if(migrations.length!==63||!migrations[0].startsWith("001_")||!migrations.at(-1).startsWith("063_"))throw new Error("expected immutable migration chain 001-063");
  ...
  console.log(JSON.stringify({result:"PASS",database:"isolated-disposable",schema:63,tests:requested.length,cleanup:"pending"}));
}finally{
