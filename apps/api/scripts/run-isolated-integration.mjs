...
try{
  ...
  const migrations=(await readdir(path.join(apiRoot,"migrations"))).filter(name=>/^\d{3}_.*\.sql$/.test(name)&&Number(name.slice(0,3))<=60).sort();
  if(migrations.length!==60||!migrations[0].startsWith("001_")||!migrations.at(-1).startsWith("060_"))throw new Error("expected immutable migration chain 001-060");
  ...
  console.log(JSON.stringify({result:"PASS",database:"isolated-disposable",schema:60,tests:requested.length,cleanup:"pending"}));
}finally{
