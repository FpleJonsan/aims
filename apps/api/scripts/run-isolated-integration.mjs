...
try{
  ...
  const migrations=(await readdir(path.join(apiRoot,"migrations"))).filter(name=>/^\d{3}_.*\.sql$/.test(name)&&Number(name.slice(0,3))<=61).sort();
  if(migrations.length!==61||!migrations[0].startsWith("001_")||!migrations.at(-1).startsWith("061_"))throw new Error("expected immutable migration chain 001-061");
  ...
  if(requested.some(name=>[".test-dist/test/document-worker-integration.test.js",".test-dist/test/recovery-generation-integration.test.js",".test-dist/test/restore-checker-integration.test.js",".test-dist/test/storage-object-version-integration.test.js"].includes(name)))adminPsql(`
  ...
  console.log(JSON.stringify({result:"PASS",database:"isolated-disposable",schema:61,tests:requested.length,cleanup:"pending"}));
}finally{
