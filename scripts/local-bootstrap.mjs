...
if(mode==='bootstrap'){
 ...
 if(version!=='71'){
  ...
  if(!Number.isInteger(current)||current<0||current>71)throw new Error(`Unsupported schema version ${version}`);
  for(const name of names){
   if(Number(name.slice(0,3))<=current)continue;
   const migration=productionMigrationSql(name,readFileSync(path.join(directory,name),'utf8'));
   if(migration){sql('SET ROLE aims_owner;\n'+migration);console.log(`Applied production-safe ${name}`)}
   else console.log(`Deferred development fixture ${name}`);
  }
 }
 ...
 console.log('Schema 71 ready; existing P6 role separation verified.');
}else{
 if(sql('SELECT version FROM aims_schema_version WHERE singleton')!=='71')throw new Error('Migrate first');
