...
try{
 ...
 console.log('Checking PostgreSQL...');const pool=new pg.Pool({connectionString:env.DATABASE_URL,connectionTimeoutMillis:3000});try{const result=await pool.query('SELECT version,migration_id FROM aims_schema_version WHERE singleton');if(result.rows[0]?.version!==71||result.rows[0]?.migration_id!=='071_p20_7a_enterprise_ui_contracts')throw new Error('Schema 71 required. Run node scripts/local-bootstrap.mjs migrate.')}finally{await pool.end()}
