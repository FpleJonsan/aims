...
try{
 ...
 console.log('Checking PostgreSQL...');const pool=new pg.Pool({connectionString:env.DATABASE_URL,connectionTimeoutMillis:3000});try{const result=await pool.query('SELECT version,migration_id FROM aims_schema_version WHERE singleton');if(result.rows[0]?.version!==70||result.rows[0]?.migration_id!=='070_p20_5h_ai_configuration_authority')throw new Error('Schema 70 required. Run node scripts/local-bootstrap.mjs migrate.')}finally{await pool.end()}
