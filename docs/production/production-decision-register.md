...
| D-008 | Redis necessity and worker architecture | CONFIRMED | Redis is not required for Production v1; use a PostgreSQL-backed independent worker for durable outbox delivery and malware scanning. Financial truth and authoritative transitions remain synchronous PostgreSQL operations. | P15 capacity may reopen Redis only with measured need; implementation requires separate authorization | Architecture / SRE | P7 implementation |
