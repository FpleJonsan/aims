...
| D-011 | Telegram in Production v1 | DECISION REQUIRED | P9 audit requires code hardening before any external setup; `DISABLED FOR V1` remains preferred unless business confirms need, privacy/destination policy and operational ownership | Business need, approved bot/channel, private-chat/group policy, support, privacy and compliance position | Finance product owner / Security / Privacy | P9 implementation / external enablement |
...
| C-011 | Telegram is an optional channel, never Approval or financial authority | CONFIRMED FOUNDATION | Telegram and web converge on current `ApprovalService.act` authority; P9 hardening must close OFF gating, provider reliability, bounds, worker configuration, destination and data-minimization gaps before enablement. |
