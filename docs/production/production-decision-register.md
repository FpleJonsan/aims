...
| D-011 | Telegram in Production v1 | DECISION REQUIRED | P9 code hardening and final review PASS; keep OFF until external approval. `DISABLED FOR V1` remains preferred unless business confirms need, approved private-chat policy, privacy and operational ownership | Business need, approved bot/channel, token custody, support, privacy and compliance position | Finance product owner / Security / Privacy | External enablement only |
...
| C-011 | Telegram is an optional channel, never Approval or financial authority | CONFIRMED FOUNDATION | Telegram and web converge on current `ApprovalService.act`; P9 implements master-OFF gating, bounded provider I/O/retry, strict configuration/input contracts, verified private-chat binding/revoke and minimized purpose projection. Final review PASS; external approval remains open. |
