...
| Migrations | Sixty-one immutable lexical SQL migrations. Schema readiness expects version 61 at `061_p13_storage_object_version_binding`. P6 provides a disposable migration/role proof and privilege manifest; a provider deployment and checksum policy remain open. Several historical migrations contain local/competition fixtures and must not be applied blindly as production bootstrap data. |
