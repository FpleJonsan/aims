...
| Migrations | Seventy-one immutable lexical SQL migrations. Schema readiness expects version 71 at `071_p20_7a_enterprise_ui_contracts`. Production uses a fail-closed execution plan that defers fixture-only migrations and checksum-pins fixture removal from two mixed historical migrations; development applies fixtures only as a separate final layer. The disposable production release proof validates the role manifest, safe defaults and zero business/demo rows. |
...
| Documents | Migration 061 and the P7 worker provide provider-neutral exact backend/key/version/SHA-256/size identity, quarantine, scanner-gated promotion and exact-version recovery contracts. The version-aware filesystem adapter is development/test-only and refuses protected environments. No Production object-storage adapter or malware engine is wired. |
