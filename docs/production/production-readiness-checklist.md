...
| PostgreSQL role/runtime foundation | PASS | Disposable clean bootstrap through schema 71, NOLOGIN ownership, executor separation including the document worker, defaults, drift checks, TLS/database identity validation and UAT pass. |
...
| Migration/bootstrap | PASS / FROZEN | `migrate:production` validates all 71 immutable files, defers 14 fixture-only migrations, checksum-pins the two mixed migrations, applies schema 71, hardens privileges and fails unless the fresh database contains zero fixture/business rows; the disposable release proof passes. |
