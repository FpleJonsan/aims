...
| **Dark mode policy** | AIMS currently forces `color-scheme: light` unconditionally, which the audit found actively overrides a user's OS-level dark-mode preference for native form controls, with no dark theme offered in exchange (M13). This standard requires an explicit choice, to be made in the P19.3+ migration: either (a) implement a real dark theme that responds to `prefers-color-scheme`/a user toggle, token-for-token consistent with §10's Contrast row, or (b) formally adopt light-only as deliberate product policy and keep `color-scheme: light` only as a documented, intentional decision — not, as today, an unexplained default with no stated policy either way. Silence is not an acceptable third option. |
