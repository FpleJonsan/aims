...
export function OperationalDrill({api,drill,open,back}:{api:PortalApi;drill:Extract<DashboardDrill,{view:"FINANCE_CONTROL"|"PAYMENT_QUEUE"}>;open:(id:string)=>Promise<void>;back:()=>void}) {
  ...
  return <section><button className="back" onClick={back}>← Finance Dashboard</button>{notice&&<p className="notice">{notice}</p>}<RequestList items={rows} open={open} empty={()=>Promise.resolve()} canCreate={false} requesterView={false}/></section>;
}
